...
["tag", "category", "suitable", "topic", "language", "keyword"].forEach(
  (filterType) => {
...
app.get("/repo/:owner/:name.html", (req, res) => {
  ...
  const item = loadData(`repo/${owner}/${name}.json`);
  const meta = loadData("meta.json") || {
    topTags: [],
    topStars: [],
    topAppearances: [],
  ...
  const summary = item.summaryData?.summary
    ? item.summaryData.summary.substring(0, 150).replace(/\n/g, " ")
    : `查看 ${item.repo} 的详细 AI 总结。`;
  ...
  if (filterType !== "keyword") {
    filteredList = list.filter((item: any) => {
      if (filterType === "topic") {
        return (
          item.githubData &&
          item.githubData.topics &&
          item.githubData.topics.some(
            (t: string) => t.toLowerCase() === lowerValue,
          )
        );
      }
      ...
      if (Array.isArray(item.summaryData[propName])) {
        return item.summaryData[propName].some(
          (t: string) => t.toLowerCase() === lowerValue,
        );
      }

      return (
        typeof item.summaryData[propName] === "string" &&
...
if (process.env.NODE_ENV !== "production" || process.env.RUN_LOCAL === "true") {
  app.listen(PORT, () => {
    console.log(`Trending running at http://localhost:${PORT}`);
  });
}
